...
| 文件 | 角色 | 部署位置 |
|------|------|---------|
| `formatA.xsl` | A 院原生 → 统一格式 | integration |
| `formatB.xsl` | B 院原生 → 统一格式 | integration |
| `formatC.xsl` | C 院原生 → 统一格式 | integration |
| `unifiedToA.xsl` | 统一格式 → A 院格式 | college-a |
...
```text
[INFO] Tests run: 23, Failures: 0, Errors: 0, Skipped: 0    (common)
...
测试覆盖：协议帧编解码、XML 读写、XSD 验证、XSL 转换、DAO 集成测试（真实 DB）、Handler 单元测试、跨院路由模拟测试。
...
- **跨院选课：** 目标院的选课表是唯一真相源，源院不存副本
- **跨院退课：** 先删目标院 → 成功后才返回给客户端
- **无 2PC：** 不实现两阶段提交，以简化实现

### 已知限制

1. 跨院选课不在源院记录反向引用，查询"某学生选了哪些外院课"需要聚合时遍历三院
2. B/C 院 ChoiceDao 缺少 `enrollFromOther` 双轨，跨院学生写入时仅 A 院保留来源标记
3. Integration Server 宕机时，客户端只能进行本院操作（降级）

### 改进方向

- 源院增加选课副本（标记 `cross=true` + `origin=B`），便于学生查自己的全量选课
...
| XML Schema (12 个) | 各院 + common + integration |
| XSL 转换文件 (7 个) | `integration/xsl/` + 各院 `xsl/` |
